...
- 학생 답의 띄어쓰기(공백) 유무는 채점에 영향을 주지 않는다. 공백을 붙이거나 떼도 나열된 뜻과 같은 단어들로 구성되어 있으면 정답이다 (예: "학교 가방" 정답에 "학교가방"이라고 붙여 써도 정답, "계정" 정답에 "계 정"이라고 띄어 써도 정답). 이는 아래 '복합명사/구의 일부 의미 요소가 빠지면 오답' 규칙과는 별개다 — 그 규칙은 실제로 뜻을 구성하는 단어 자체가 빠졌을 때만 적용한다 (예: "느리게 읽는 사람" 정답에 "느리다"만 쓴 경우).
- 품사가 명백히 다르면 오답 (예: 동사 뜻 자리에 형용사/상태를 나타내는 답을 쓰면 오답). 단, 한 단어에 나열된 뜻이 여러 개이고 그 뜻들의 품사가 서로 다른 경우(예: book = 책(명사)/예약하다(동사)), 학생 답이 그 중 하나의 뜻과 (표현 차이를 감안해도) 일치하면 그 뜻의 품사를 기준으로 판단한다. 나열된 다른 뜻의 품사와 다르다는 이유로 오답 처리하지 않는다.
